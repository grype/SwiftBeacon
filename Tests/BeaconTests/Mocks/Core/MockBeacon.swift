...
public class MockBeacon: Beacon, Cuckoo.ClassMock {
    ...
    return cuckoo_manager.call("logsSignals(ofType: T.Type) -> Bool",
            parameters: (aType),
    ...
    return cuckoo_manager.call("signal(_: Signal)",
            parameters: (aSignal),
    ...
    return cuckoo_manager.call("when(_: T.Type, subscriber: AnyObject?, do: @escaping (T, Announcer)->Void)",
    ...
    return cuckoo_manager.call("remove(subscription: Subscription<T>)",
    ...
    return cuckoo_manager.call("unsubscribe(_: AnyObject)",
            parameters: (anObject),
	...
	public struct __StubbingProxy_Beacon: Cuckoo.StubbingProxy {
	    ...
	    func logsSignals<M1: Cuckoo.Matchable, T: Signal>(ofType aType: M1) -> Cuckoo.ClassStubFunction<(T.Type), Bool> where M1.MatchedType == T.Type {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockBeacon.self, method: "logsSignals(ofType: T.Type) -> Bool", parameterMatchers: matchers))
	    }
	    ...
	    func signal<M1: Cuckoo.Matchable>(_ aSignal: M1) -> Cuckoo.ClassStubNoReturnFunction<(Signal)> where M1.MatchedType == Signal {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockBeacon.self, method: "signal(_: Signal)", parameterMatchers: matchers))
	    }
	    ...
	    func when<M1: Cuckoo.Matchable, M2: Cuckoo.OptionalMatchable, M3: Cuckoo.Matchable, T: Announceable>(_ aType: M1, subscriber: M2, do aBlock: M3) -> Cuckoo.ClassStubNoReturnFunction<(T.Type, AnyObject?, (T, Announcer)->Void)> where M1.MatchedType == T.Type, M2.OptionalMatchedType == AnyObject, M3.MatchedType == (T, Announcer)->Void {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockBeacon.self, method: "when(_: T.Type, subscriber: AnyObject?, do: @escaping (T, Announcer)->Void)", parameterMatchers: matchers))
	    }
	    ...
	    func remove<M1: Cuckoo.Matchable, T: Announceable>(subscription: M1) -> Cuckoo.ClassStubNoReturnFunction<(Subscription<T>)> where M1.MatchedType == Subscription<T> {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockBeacon.self, method: "remove(subscription: Subscription<T>)", parameterMatchers: matchers))
	    }
	    ...
	    func unsubscribe<M1: Cuckoo.Matchable>(_ anObject: M1) -> Cuckoo.ClassStubNoReturnFunction<(AnyObject)> where M1.MatchedType == AnyObject {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockBeacon.self, method: "unsubscribe(_: AnyObject)", parameterMatchers: matchers))
	    }
	...
	public struct __VerificationProxy_Beacon: Cuckoo.VerificationProxy {
	    ...
	    func logsSignals<M1: Cuckoo.Matchable, T: Signal>(ofType aType: M1) -> Cuckoo.__DoNotUse<(T.Type), Bool> where M1.MatchedType == T.Type {
	        let matchers: [Cuckoo.ParameterMatcher<(T.Type)>] = [wrap(matchable: aType) { $0 }]
	        return cuckoo_manager.verify("logsSignals(ofType: T.Type) -> Bool", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func signal<M1: Cuckoo.Matchable>(_ aSignal: M1) -> Cuckoo.__DoNotUse<(Signal), Void> where M1.MatchedType == Signal {
	        let matchers: [Cuckoo.ParameterMatcher<(Signal)>] = [wrap(matchable: aSignal) { $0 }]
	        return cuckoo_manager.verify("signal(_: Signal)", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    ...
	    func when<M1: Cuckoo.Matchable, M2: Cuckoo.OptionalMatchable, M3: Cuckoo.Matchable, T: Announceable>(_ aType: M1, subscriber: M2, do aBlock: M3) -> Cuckoo.__DoNotUse<(T.Type, AnyObject?, (T, Announcer)->Void), Void> where M1.MatchedType == T.Type, M2.OptionalMatchedType == AnyObject, M3.MatchedType == (T, Announcer)->Void {
	        let matchers: [Cuckoo.ParameterMatcher<(T.Type, AnyObject?, (T, Announcer)->Void)>] = [wrap(matchable: aType) { $0.0 }, wrap(matchable: subscriber) { $0.1 }, wrap(matchable: aBlock) { $0.2 }]
	        return cuckoo_manager.verify("when(_: T.Type, subscriber: AnyObject?, do: @escaping (T, Announcer)->Void)", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    ...
	    func remove<M1: Cuckoo.Matchable, T: Announceable>(subscription: M1) -> Cuckoo.__DoNotUse<(Subscription<T>), Void> where M1.MatchedType == Subscription<T> {
	        let matchers: [Cuckoo.ParameterMatcher<(Subscription<T>)>] = [wrap(matchable: subscription) { $0 }]
	        return cuckoo_manager.verify("remove(subscription: Subscription<T>)", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func unsubscribe<M1: Cuckoo.Matchable>(_ anObject: M1) -> Cuckoo.__DoNotUse<(AnyObject), Void> where M1.MatchedType == AnyObject {
	        let matchers: [Cuckoo.ParameterMatcher<(AnyObject)>] = [wrap(matchable: anObject) { $0 }]
	        return cuckoo_manager.verify("unsubscribe(_: AnyObject)", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
