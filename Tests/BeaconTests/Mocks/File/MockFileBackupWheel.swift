...
public class MockFileBackupWheel: FileBackupWheel, Cuckoo.ClassMock {
    ...
    return try cuckoo_manager.callThrows("rotate(fileAt: URL) throws",
            parameters: (aURL),
    ...
    return try cuckoo_manager.callThrows("backupFile(at: URL) throws",
            parameters: (aURL),
            escapingParameters: (aURL),
            superclassCall:
    ...
    return try cuckoo_manager.callThrows("deleteOldBackupsOfFile(at: URL) throws",
            parameters: (aURL),
            escapingParameters: (aURL),
            superclassCall:
    ...
    return try cuckoo_manager.callThrows("backupsOfFile(at: URL) throws -> [URL]",
            parameters: (aURL),
            escapingParameters: (aURL),
            superclassCall:
    ...
    return cuckoo_manager.call("nextBackupURLFor(url: URL) -> URL",
            parameters: (url),
    ...
    return cuckoo_manager.call("fileExists(at: URL) -> Bool",
            parameters: (url),
            escapingParameters: (url),
            superclassCall:
    ...
    return cuckoo_manager.call("fileSize(at: URL) -> UInt64",
            parameters: (url),
            escapingParameters: (url),
            superclassCall:
	...
	public struct __StubbingProxy_FileBackupWheel: Cuckoo.StubbingProxy {
	    ...
	    func rotate<M1: Cuckoo.Matchable>(fileAt aURL: M1) -> Cuckoo.ClassStubNoReturnThrowingFunction<(URL)> where M1.MatchedType == URL {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "rotate(fileAt: URL) throws", parameterMatchers: matchers))
	    }
	    
	    func backupFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.ClassStubNoReturnThrowingFunction<(URL)> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "backupFile(at: URL) throws", parameterMatchers: matchers))
	    }
	    ...
	    func deleteOldBackupsOfFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.ClassStubNoReturnThrowingFunction<(URL)> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "deleteOldBackupsOfFile(at: URL) throws", parameterMatchers: matchers))
	    }
	    
	    func backupsOfFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.ClassStubThrowingFunction<(URL), [URL]> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "backupsOfFile(at: URL) throws -> [URL]", parameterMatchers: matchers))
	    }
	    ...
	    func nextBackupURLFor<M1: Cuckoo.Matchable>(url: M1) -> Cuckoo.ClassStubFunction<(URL), URL> where M1.MatchedType == URL {
	        ...
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "nextBackupURLFor(url: URL) -> URL", parameterMatchers: matchers))
	    }
	    
	    func fileExists<M1: Cuckoo.Matchable>(at url: M1) -> Cuckoo.ClassStubFunction<(URL), Bool> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: url) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "fileExists(at: URL) -> Bool", parameterMatchers: matchers))
	    }
	    
	    func fileSize<M1: Cuckoo.Matchable>(at url: M1) -> Cuckoo.ClassStubFunction<(URL), UInt64> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: url) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockFileBackupWheel.self, method: "fileSize(at: URL) -> UInt64", parameterMatchers: matchers))
	    }
	...
	public struct __VerificationProxy_FileBackupWheel: Cuckoo.VerificationProxy {
	    ...
	    func rotate<M1: Cuckoo.Matchable>(fileAt aURL: M1) -> Cuckoo.__DoNotUse<(URL), Void> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return cuckoo_manager.verify("rotate(fileAt: URL) throws", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func backupFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.__DoNotUse<(URL), Void> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return cuckoo_manager.verify("backupFile(at: URL) throws", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func deleteOldBackupsOfFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.__DoNotUse<(URL), Void> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return cuckoo_manager.verify("deleteOldBackupsOfFile(at: URL) throws", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func backupsOfFile<M1: Cuckoo.Matchable>(at aURL: M1) -> Cuckoo.__DoNotUse<(URL), [URL]> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: aURL) { $0 }]
	        return cuckoo_manager.verify("backupsOfFile(at: URL) throws -> [URL]", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func nextBackupURLFor<M1: Cuckoo.Matchable>(url: M1) -> Cuckoo.__DoNotUse<(URL), URL> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: url) { $0 }]
	        return cuckoo_manager.verify("nextBackupURLFor(url: URL) -> URL", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func fileExists<M1: Cuckoo.Matchable>(at url: M1) -> Cuckoo.__DoNotUse<(URL), Bool> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: url) { $0 }]
	        return cuckoo_manager.verify("fileExists(at: URL) -> Bool", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
	    
	    @discardableResult
	    func fileSize<M1: Cuckoo.Matchable>(at url: M1) -> Cuckoo.__DoNotUse<(URL), UInt64> where M1.MatchedType == URL {
	        let matchers: [Cuckoo.ParameterMatcher<(URL)>] = [wrap(matchable: url) { $0 }]
	        return cuckoo_manager.verify("fileSize(at: URL) -> UInt64", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
